...
{
    view = HMM_LookAt_RH(this->mp_Object->transform.Position(), this->mp_Object->transform.Position() + this->mp_Object->transform.Forward(), this->mp_Object->transform.Up());
}
...
void CameraComponent::SetCameraMode(bool orthographic)
{
    ...
    if(orthographic)
        projection = HMM_Orthographic_RH_NO(-orthographicSize.X, orthographicSize.X, -orthographicSize.Y, orthographicSize.Y, nearClipPlane, farClipPlane);
    else
        projection = HMM_Perspective_RH_NO(fov, aspectRatio, nearClipPlane, farClipPlane);
}
